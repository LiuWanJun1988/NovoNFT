...
contract NOVO is INOVO, Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    mapping(address => Stake) private _stakingMap;
    ...
    address[] private _stakingHolders;
    ...
    // staking functions
    function staking(address _address, uint256 _amount) public {
        require(
            msg.sender == address(_novoNFT),
            "Should be called by only NFT"
        );
        _stakingMap[_address] = Stake(_amount, 0, 0);
        _stakingHolders.push(_address);
    }

    function removeStakingHolder(address _address) internal {
        uint256 size = _stakingHolders.length;
        if (size == 0) return;

        for (uint256 i = 0; i < size; i++) {
            if (_stakingHolders[i] == _address) {
                _stakingHolders[i] = _stakingHolders[size - 1];
                _stakingHolders.pop();
                return;
            }
        }
    ...
    function unstaking(address _address) public {
        require(
            msg.sender == address(_novoNFT),
            "Should be called by only NFT"
        );
        require(_stakingMap[_address].staked > 0, "No staked address");
        delete _stakingMap[_address];
        removeStakingHolder(_address);
    }

    function isStaking(address _address) public view returns (bool) {
        return _stakingMap[_address].staked > 0;
    }

    function getTotalStakedAmount() public view returns (uint256) {
        uint256 totalStakingAmount = 0;
        for (uint256 i = 0; i < _stakingHolders.length; i++) {
            totalStakingAmount += _stakingMap[_stakingHolders[i]].staked;
        }
    ...
    function getTotalRewardAmount() public view returns (uint256) {
        ...
        for (uint256 i = 0; i < _stakingHolders.length; i++) {
            totalRewardAmount +=
                _stakingMap[_stakingHolders[i]].bagSizeReward +
                _stakingMap[_stakingHolders[i]].stakingTimeReward;
        }
    ...
    function addBagSizeReward(address _address, uint256 _amount) internal {
        _stakingMap[_address].bagSizeReward += _amount;
    }

    function addStakingTimeReward(address _address, uint256 _amount) internal {
        _stakingMap[_address].stakingTimeReward += _amount;
    }
